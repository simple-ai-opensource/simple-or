...
#
# class Tindar:
#     """Class to solve maximum matching problem
#
#     Args:
...
#         if m != n:
#             raise ValueError(
#                 f"love_matrix is not square: love_matrix.shape" f"= {love_matrix.shape}"
#             )
#
#         # diagonal zero check
#         for i in range(n):
...
#
#                     while not done:
#                         mutual_interest = (self.love_matrix[i, j] == 1) and (
...
#         n,
#         connectedness=None,
#         nan_probability=None,
#         generation_kind="simple",
#         attractiveness_distr="uniform",
#         unif_low=UNIF_LOW,
#         unif_high=UNIF_HIGH,
#     ):
...
#     def __repr__(self):
#         return (
#             f"Tindar problem with n={self.n}, connectedness= "
...
#             raise ValueError(f"TindarGenerator init error: " f"type(n) = {type(n)}")
#         if n <= 0:
#             raise ValueError(f"TindarGenerator init error: " f"n={n} < 0")
#
#         # connectedness
#         if not (isinstance(connectedness, (int, float)) or connectedness is None):
#             raise ValueError(
#                 f"TindarGenerator init error: "
...
#         return love_matrix
#
#     def create_love_matrix(
#         self,
#         n: int = None,
#         connectedness: Optional[int] = None,
...
#         [TindarGenerator(p[0], p[1]) for _ in range(repeat)] for p in parameters
#     ]
#     tindar_problems = [item for sublist in tindar_problems_nested for item in sublist]
#     tindars = [
#         Tindar(tindar_problem=tindar_problem) for tindar_problem in tindar_problems
#     ]
#
#     results = []
#     counter = 1
...
#                 result = {**result, "love_matrix": tp.love_matrix, "solution": sol}
#
#             if verbose:
